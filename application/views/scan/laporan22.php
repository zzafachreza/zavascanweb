    ...
    if(!empty($_POST)){
	   ...
	   if($ekspedisi=="SEMUA"){
	        ...
	        if($status_barcode=="SEMUA"){
	            $sql="SELECT ekspedisi,nama,barcode2.tanggal,jam,barcode2.id id,nama_lengkap,status_barcode FROM barcode2 join member ON member.id = barcode2.id_member  WHERE id_member='$id_member' AND barcode2.tanggal between '$awal' AND '$akhir' ORDER BY barcode2.id ASC";     
	        }else{
	            $sql="SELECT ekspedisi,nama,barcode2.tanggal,jam,barcode2.id id,nama_lengkap,status_barcode FROM barcode2 join member ON member.id = barcode2.id_member  WHERE id_member='$id_member' AND barcode2.tanggal between '$awal' AND '$akhir' AND status_barcode='OUT' ORDER BY barcode2.id ASC";
	        }
	        ...
	        if($status_barcode=="SEMUA"){
	            $sql="SELECT ekspedisi,nama,barcode2.tanggal,jam,barcode2.id id,nama_lengkap,status_barcode FROM barcode2 join member ON member.id = barcode2.id_member  WHERE id_member='$id_member' AND barcode2.ekspedisi='$ekspedisi' AND barcode2.tanggal between '$awal' AND '$akhir' ORDER BY barcode2.id ASC";
	        }else{
	            $sql="SELECT ekspedisi,nama,barcode2.tanggal,jam,barcode2.id id,nama_lengkap,status_barcode FROM barcode2 join member ON member.id = barcode2.id_member  WHERE id_member='$id_member' AND barcode2.ekspedisi='$ekspedisi' AND barcode2.tanggal between '$awal' AND '$akhir' AND status_barcode='OUT' ORDER BY barcode2.id ASC";
	        }
		  		...
		  		?>
		  			<tr>
		  				...
		  				<td><?php echo $row->nama_lengkap ?></td>
		  					...
		  					<a href="<?php echo site_url().'laporan2/hapus/'.$row->id.'/'.$awal.'/'.$akhir ?>" class="btn btn-danger text-white"><i class="flaticon-delete"></i></a>	
