    ...
    if(!empty($_POST)){
	   ...
	   if($ekspedisi=="SEMUA"){
	        ...
	        if($status_barcode=="SEMUA"){
	            $sql="SELECT ekspedisi,customer,nama,barcode.tanggal,jam,barcode.id id,nama_lengkap,status_barcode,data_cek.barcode_oke FROM barcode join member ON member.id = barcode.id_member LEFT JOIN data_cek ON barcode.nama = data_cek.barcode_oke  WHERE id_member='$id_member' AND barcode.tanggal between '$awal' AND '$akhir' ORDER BY barcode.id ASC";     
	        }else{
	            $sql="SELECT ekspedisi,customer,nama,barcode.tanggal,jam,barcode.id id,nama_lengkap,status_barcode,data_cek.barcode_oke FROM barcode join member ON member.id = barcode.id_member LEFT JOIN data_cek ON barcode.nama = data_cek.barcode_oke  WHERE id_member='$id_member' AND barcode.tanggal between '$awal' AND '$akhir' AND status_barcode='OUT' ORDER BY barcode.id ASC";
	        }
	        ...
	        if($status_barcode=="SEMUA"){
	            $sql="SELECT customer,ekspedisi,nama,barcode.tanggal,jam,barcode.id id,nama_lengkap,status_barcode,data_cek.barcode_oke FROM barcode join member ON member.id = barcode.id_member LEFT JOIN data_cek ON barcode.nama = data_cek.barcode_oke  WHERE id_member='$id_member' AND barcode.ekspedisi='$ekspedisi' AND barcode.tanggal between '$awal' AND '$akhir' ORDER BY barcode.id ASC";
	        }else{
	            $sql="SELECT customer,ekspedisi,nama,barcode.tanggal,jam,barcode.id id,nama_lengkap,status_barcode,data_cek.barcode_oke FROM barcode join member ON member.id = barcode.id_member LEFT JOIN data_cek ON barcode.nama = data_cek.barcode_oke  WHERE id_member='$id_member' AND barcode.ekspedisi='$ekspedisi' AND barcode.tanggal between '$awal' AND '$akhir' AND status_barcode='OUT' ORDER BY barcode.id ASC";
	        }
		  			...
		  			<tr>
		  				...
		  				<td><?php echo $row->customer ?></td>
		  					...
		  					<td><?php 
		  					 ...
		  					 if($row->barcode_oke !== null){
                	  			 
                                            
                	  			          $icon = '<i class="flaticon2-correct" style="font-size:x-large;color:green"></i> Sudah Upload';
                	  			        }else{
                	  			  
                	  			        
                	  			            $icon = '<i class="flaticon2-cross" style="font-size:large;color:red"></i> Belum';
                	  			        }
